...
public protocol MarketDataService {
    ...
    ///
    ///  - Parameter figi: Figi идентификаторы инструментов.
    ///
    ...
    func getLastPrices(figi: [String]) throws -> EventLoopFuture<[LastPrice]> {
        self.client
            .getLastPrices(
                Requests
                    .MarketDataServiceRequests
                    .getLastPricesRequest.with(figi: figi)
            )
            .response
    ...
    func getOrderBook(figi: String, depth: Int32) throws -> EventLoopFuture<OrderBookInfo> {
        self.client
            .getOrderBook(
                Requests
                    .MarketDataServiceRequests
                    .getOrderBookRequest.with(figi: figi, depth: depth)
            )
            .response
            .map { $0.toModel() }
    }
    ...
    func getTradingStatus(figi: String) throws -> EventLoopFuture<TradingStatusInfo> {
        self.client
            .getTradingStatus(
                Requests
                    .MarketDataServiceRequests
                    .getTradingStatusRequest.with(figi: figi)
            )
            .response
    ...
    func getLastTrades(
        ...
        self.client
            .getLastTrades(
                Requests
                    .MarketDataServiceRequests
                    .getLastTradesRequest.with(figi: figi, from: from, to: to)
            )
            .response
    ...
    func getClosePrices(figi: [String]) throws -> EventLoopFuture<[InstrumentClosePrice]> {
        self.client
            .getClosePrices(
                Requests
                    .MarketDataServiceRequests
                    .getClosePricesRequest.with(figi: figi)
            )
            .response
